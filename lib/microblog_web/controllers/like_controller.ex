defmodule MicroblogWeb.LikeController do
  ...
  def index(conn, %{"message_id" => message_id}) do
    likes = Activity.list_message_likes(message_id)
    render(conn, "index.json", likes: likes)
  end

  def index(conn, _params) do
    likes = Activity.list_likes()
    render(conn, "index.json", likes: likes)
  end

  def create(conn, %{"like" => like_params}) do
    with {:ok, %Like{} = like} <- Activity.create_like(like_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "like" => like_params}) do
    ...
    with {:ok, %Like{} = like} <- Activity.update_like(like, like_params) do
      render(conn, "show.json", like: like)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    like = Activity.get_like!(id)
    with {:ok, %Like{}} <- Activity.delete_like(like) do
